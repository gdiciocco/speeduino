...
// Parameterised suites build their test names at run time and call
// UnityDefaultTestRun() directly. They must feed the watchdog too, or a hang
// in one of them is exactly the case the watchdog cannot catch - which is how
// test_pages took the board out twice.
#define RUN_TEST_NAMED(func, name, line) \
  do { \
